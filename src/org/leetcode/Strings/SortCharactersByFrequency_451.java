    ...
    public String frequencySort1(String s) {
        ...
        for (char c : s.toCharArray()) {
            if (!charMap.containsKey(c))
                charMap.put(c, String.valueOf(c));
            else
                charMap.put(c, charMap.get(c) + c);
        }

        PriorityQueue<Map.Entry<Character, String>> priorityQueue = new PriorityQueue<Map.Entry<Character, String>>((a, b) -> {
        ...
        while (!priorityQueue.isEmpty()) {
            output.append(priorityQueue.poll().getValue());
        }
